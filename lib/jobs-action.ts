...
export async function findJobsForMajor(major: string): Promise<Job[]> {
  try {
    ...
    const jobDetails = extractJobDetailsFromWikipediaResults(searchResults, normalizedMajor)
    ...
    if (altResponse.ok) {
      const altData: WikipediaResponse = await altResponse.json()
      const additionalDetails = extractJobDetailsFromWikipediaResults(altData.query.search, normalizedMajor)

      const combinedDetails = [...new Set([...jobDetails, ...additionalDetails])]
...
function extractJobDetailsFromWikipediaResults(results: WikipediaSearchResult[], major: string): Job[] {
  ...
  for (const result of results) {
    ...
    if (jobTitle) {
      const description = snippet.replace(/<span.*?>/g, "").replace(/<\/span>/g, "")
...
function getFallbackJobsForMajor(major: string): Job[] {
  ...
  const fallbackJobs = majorJobDatabase[normalizedMajor] || majorJobDatabase["general"]
